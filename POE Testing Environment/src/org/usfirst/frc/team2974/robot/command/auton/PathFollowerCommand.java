...
public class PathFollowerCommand extends Command {
	 ...
	 * @param smartDashboardPaths the SmartDashboard string keys where the paths should be retrieved
	 * from. If there is more than one path (more than one key) if will append all the poses
	 * together.
	 */
	 ...
	 * @param vCruise the velocity to try to reach and stay at the longest
	 * @param aMax the acceleration/deceleration of the robot
	 * @param isForwards if the robot should execute the path facing forwards or backwards
	...
	private void findAndRun() {
		...
		try {
			if (poses == null) {
				poses =
					Arrays.stream(
						smartDashboardPaths)              // loops through smartdashboard keys
						.map(
							PointRetriever::retrieveSmartDashboardPoses)         // uses the retrievePoses to convert the key into a Pose[]
						.reduce(
							new ArrayList<>(), (poses23, poses2) -> {   //
								Collections
									.addAll(poses23,
										poses2);    // adds elements of one array to a list
								return poses23;
							},
