...
public class MotionPathFollower extends MotionProvider {
	 ...
	 *
	 * @param vCruise velocity to cruise at (velocity to try to reach and stay at as long as
	 * possible)
	...
	public final Pose evaluatePose(double s) {
		...
		if (index + 1 >= robotPoses.length) {
			return robotPoses[robotPoses.length - 1];
		}

		Pose last = robotPoses[index];
		Pose next = robotPoses[index + 1];

		double localPercent = (s - index / (robotPoses.length - 1)) * (robotPoses.length - 1);
