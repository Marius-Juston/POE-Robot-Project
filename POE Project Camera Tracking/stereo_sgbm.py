...
camera_to_use = cv2.CAP_ANY;  # for the Xiema cameras (opencv built with driver)
...
while not do_calibration:
    ...
    retR, cornersL = cv2.findChessboardCorners(grayL, (patternX, patternY), None,
                                               cv2.CALIB_CB_ADAPTIVE_THRESH | cv2.CALIB_CB_FAST_CHECK | cv2.CALIB_CB_NORMALIZE_IMAGE);
    retL, cornersR = cv2.findChessboardCorners(grayR, (patternX, patternY), None,
                                               cv2.CALIB_CB_ADAPTIVE_THRESH | cv2.CALIB_CB_FAST_CHECK | cv2.CALIB_CB_NORMALIZE_IMAGE);
...
import os
if not os.path.exists("undistorted calibration.npz"):

    print("Left - intrinsic calibration...")
    ret, mtxL, distL, rvecsL, tvecsL = cv2.calibrateCamera(objpoints, imgpointsL, grayL.shape[::-1], None, None);

    print("Right - intrinsic calibration...")
    ret, mtxR, distR, rvecsR, tvecsR = cv2.calibrateCamera(objpoints, imgpointsR, grayR.shape[::-1], None, None);

    np.savez("undistorted calibration.npz", mtxL=mtxL , distL=distL, mtxR=mtxR, distR=distR, objpoints=objpoints, imgpointsL=imgpointsL, imgpointsR=imgpointsL)
else:
    matrix = np.load("undistorted calibration.npz")

    mtxL, distL,mtxR, distR, objpoints, imgpointsL, imgpointsR = [matrix[i] for i in ('mtxL', 'distL','mtxR', 'distR', 'objpoints', 'imgpointsL', 'imgpointsR')]
...
if not os.path.exists("extrinsic calibration.npz"):
    # (rms_stereo, camera_matrix_l, dist_coeffs_l, camera_matrix_r, dist_coeffs_r, R, T, E, F) = \
    #     cv2.stereoCalibrate(objpoints, imgpointsL, imgpointsR, mtxL, distL, mtxR, distR, grayL.shape[::-1],
    ...
    (rms_stereo, camera_matrix_l, dist_coeffs_l, camera_matrix_r, dist_coeffs_r, R, T, E, F) = \
        cv2.stereoCalibrate(objpoints, imgpointsL, imgpointsR, mtxL, distL, mtxR, distR, grayL.shape[::-1],
                            criteria=termination_criteria_extrinsics, flags=0)
    np.savez("extrinsic calibration.npz", rms_stereo=rms_stereo, camera_matrix_l=camera_matrix_l, dist_coeffs_l=dist_coeffs_l, camera_matrix_r=camera_matrix_r, dist_coeffs_r=dist_coeffs_r, R=R, T=T, E=E, F=F)
else:
    matrix = np.load("extrinsic calibration.npz")

    rms_stereo, camera_matrix_l, dist_coeffs_l, camera_matrix_r, dist_coeffs_r, R, T, E, F = [matrix[i] for i in ('rms_stereo', 'camera_matrix_l', 'dist_coeffs_l', 'camera_matrix_r', 'dist_coeffs_r', 'R', 'T', 'E', 'F')]
...
mapL1, mapL2 = cv2.initUndistortRectifyMap(camera_matrix_l, dist_coeffs_l, RL, PL, grayL.shape[::-1], cv2.CV_32F)
mapR1, mapR2 = cv2.initUndistortRectifyMap(camera_matrix_r, dist_coeffs_r, RR, PR, grayL.shape[::-1], cv2.CV_32F)

np.savez("undistorted rectified calibration.npz", mapL1=mapL1, mapL2=mapL2, mapR1=mapR1, mapR2=mapR2)
